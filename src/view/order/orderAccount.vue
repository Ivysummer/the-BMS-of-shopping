  ...
  export default {
    ...
    methods: {
      ...
      handleSearch () {
        ...
        console.log("已进入查询")
        var vm = this ;
        for(var i = 0;i < vm.list.length+1;i++){//二次筛选
           for(var item of vm.list){//一次筛选
              if(vm.listQuery.orderId != item.orderId){
                var index = vm.list.indexOf(item);
                console.log(index)
                vm.list.splice(index,1);//把index和id不相等的记录删除，删位置index
              }
            }
        }
